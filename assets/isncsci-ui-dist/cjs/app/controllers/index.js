...
var core_helpers_index = require('../../core/helpers/index.js');
var examData_helper = require('../../examData.helper-43fb7cf8.js');
require('../../core/boundaries/index.js');
...
const allCellsHaveSameValues = (selectedCells) => {
    ...
    return selectedCells.every((cell) => core_helpers_index.cellsMatch(cell, firstCell));
};
class InputLayoutController {
    ...
    handleValueInput(value) {
        return getNextActiveCell_useCase.__awaiter(this, void 0, void 0, function* () {
            const state = app_store_index.appStore.getState();
            if (!state.activeCell) {
                return;
            }
            const propagateDown = true;
            const result = yield getNextActiveCell_useCase.setCellsValueUseCase(value, state.selectedCells.slice(), state.gridModel.slice(), state.vac, state.dap, state.rightLowestNonKeyMuscleWithMotorFunction, state.leftLowestNonKeyMuscleWithMotorFunction, state.comments, propagateDown, this.appStoreProvider, this.externalMessageProvider);
            ...
            if (result.updatedCells.length > 1) {
                ...
                if (!isStar) {
                    ...
                    const nextActiveCell = getNextActiveCell_useCase.getNextActiveCellUseCase(state.activeCell.name, state.gridModel);
                    getNextActiveCell_useCase.setActiveCellUseCase(nextActiveCell, state.activeCell, 'single', state.selectedCells, state.gridModel.slice(), this.appStoreProvider);
                }
            }
        });
    }
    ...
    updateInputButtons(activeCell, selectedCells = [], inputButtons, considerNormal, reasonImpairmentNotDueToSci, reasonImpairmentNotDueToSciSpecify) {
        ...
        if (activeCell) {
            ...
            if (examData_helper.sensoryCellRegex.test(activeCell.name)) {
    ...
    vacDap_onChange() {
        ...
        getNextActiveCell_useCase.setVacDapUseCase(state.gridModel, vac, dap, state.rightLowestNonKeyMuscleWithMotorFunction, state.leftLowestNonKeyMuscleWithMotorFunction, state.comments, this.appStoreProvider, this.externalMessageProvider);
    }
    extraInputs_onChange() {
        ...
        const state = app_store_index.appStore.getState();
        const cellComments = examData_helper.getCellComments(state.gridModel);
        getNextActiveCell_useCase.setExtraInputsUseCase(state.gridModel.slice(), state.vac, state.dap, this.rightLowest.value, this.leftLowest.value, this.comments.value, cellComments, this.appStoreProvider, this.externalMessageProvider);
    }
    updateCellCommentsDisplay(gridModel) {
        if (this.cellCommentsDisplay) {
            this.cellCommentsDisplay.innerHTML = '';
            const cellComments = examData_helper.getCellComments(gridModel);
    ...
    stateChanged(state, actionType) {
        ...
        switch (actionType) {
            case app_store_index.Actions.SET_GRID_MODEL:
                this.updateView(state.gridModel.slice());
                this.updateCellCommentsDisplay(state.gridModel);
                break;
            case app_store_index.Actions.SET_TOTALS:
            case app_store_index.Actions.CLEAR_TOTALS_AND_ERRORS:
                this.updateTotals(state.totals);
                break;
            case app_store_index.Actions.SET_ACTIVE_CELL:
                ...
                break;
            case app_store_index.Actions.SET_CELLS_VALUE:
                this.updateCellViews(state.updatedCells.slice());
                this.updateInputButtons(state.activeCell, state.selectedCells, this.inputButtons, this.considerNormal, this.reasonImpairmentNotDueToSci, this.reasonImpairmentNotDueToSciSpecify);
                this.updateCellCommentsDisplay(state.gridModel);
                break;
            case app_store_index.Actions.SET_VAC_DAP:
                this.updateDropdowns(state.vac, state.dap);
                break;
            case app_store_index.Actions.SET_EXTRA_INPUTS:
    ...
    grid_onClick(e) {
        ...
        let state = app_store_index.appStore.getState();
        ...
        if (this.multipleSelectionEnabled) {
            if (!isCtrlPressed) {
                ...
                if (state.activeCell && state.selectedCells.length > 1) {
                    ...
                    state = app_store_index.appStore.getState();
                }
            }
            ...
            const selectionMode = isCtrlPressed
                ...
                : state.activeCell
                    ? 'range'
                    : 'single';
            getNextActiveCell_useCase.setActiveCellUseCase(name, state.activeCell, selectionMode, state.selectedCells, state.gridModel.slice(), this.appStoreProvider);
        }
        else {
            ...
            getNextActiveCell_useCase.setActiveCellUseCase(name, null, // No active cell for range selection
...
class KeyPointDiagramController {
    ...
    updateRowKeyPoints(row, keyPointsDiagram) {
        var _a, _b;
        const levelNameExec = examData_helper.levelNameRegex.exec((_b = (_a = row[1]) === null || _a === void 0 ? void 0 : _a.name) !== null && _b !== void 0 ? _b : '');
...
exports.InputLayoutController = InputLayoutController;
exports.KeyPointDiagramController = KeyPointDiagramController;
