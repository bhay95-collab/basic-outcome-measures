...
const setActiveCellUseCase = (cellName, currentActiveCell, selectionMode, currentCellsSelected, gridModel, appStoreProvider) => __awaiter(void 0, void 0, void 0, function* () {
    // 1. Get the cell from the grid model.
    const cell = cellName === null ? null : examData_helper.findCell(cellName, gridModel);
    // 2. If the selection mode is `single`,
    ...
    if (selectionMode === 'range') {
        ...
        // 4.2. Produce the range between the active cell and the new cell.
        const { motorRange, sensoryRange } = examData_helper.getCellRange(examData_helper.getCellPosition(currentActiveCell.name), examData_helper.getCellPosition(cell.name), gridModel);
...
const setCellsValueUseCase = (value, selectedCells, gridModel, vac, dap, rightLowestNonKeyMuscle, leftLowestNonKeyMuscle, comments, propagateDown, appStoreProvider, externalMessageProvider) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!examData_helper.motorValueRegex.test(value)) {
        throw new Error(`Invalid value: ${value}`);
    }
    const isSensoryValue = examData_helper.sensoryValueRegex.test(value);
    let cellsToUpdate = [];
    // 2. Determine if an error message needs to be added for values flagged with a star.
    ...
    if (selectedCells.length === 1 && propagateDown) {
        ...
        if (examData_helper.sensoryCellRegex.test(selectedCells[0].name) && !isSensoryValue) {
            return { updatedCells: [] };
        }
        // 3.3. Get the range of cells to update.
        const { motorRange, sensoryRange } = examData_helper.getCellRange(examData_helper.getCellPosition(selectedCells[0].name), null, gridModel, true);
        ...
        selectedCells.forEach((selectedCell) => {
            if (isSensoryValue || examData_helper.motorCellRegex.test(selectedCell.name)) {
    ...
    try {
        ...
        // 8. Update external listeners
        const { examData } = examData_helper.getExamDataFromGridModel(gridModel, vac, dap, rightLowestNonKeyMuscle, leftLowestNonKeyMuscle, comments);
...
const setExtraInputsUseCase = (gridModel, vac, dap, rightLowestNonKeyMuscleWithMotorFunction, leftLowestNonKeyMuscleWithMotorFunction, comments, cellComments, appStoreProvider, externalMessageProvider) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const { examData } = examData_helper.getExamDataFromGridModel(gridModel, vac, dap, rightLowestNonKeyMuscleWithMotorFunction, leftLowestNonKeyMuscleWithMotorFunction, comments);
    try {
        yield externalMessageProvider.sendOutExamData(examData);
    }
    catch (error) {
...
const getDownPropagationCellRange = (cell, gridModel) => {
    const column = examData_helper.getCellColumn(cell.name);
    let continueDown = true;
    let rowIndex = examData_helper.getCellRow(cell.name) + 1;
    const range = [cell];
    while (continueDown && rowIndex < gridModel.length) {
        ...
        if (core_helpers_index.cellsMatch(nextCell, cell)) {
            range.push(nextCell);
        }
        else {
            continueDown = false;
...
const setStarDetailsUseCase = (considerNormal, reason, details, selectedCells, gridModel, vac, dap, rightLowestNonKeyMuscleWithMotorFunction, leftLowestNonKeyMuscleWithMotorFunction, comments, propagateDown, appStoreProvider, externalMessageProvider) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const mismatch = selectedCells.find((selectedCell) => !core_helpers_index.cellsMatch(selectedCell, referenceCell));
    if (mismatch) {
        // 3.1. If there are mismatch, we throw an error.
...
exports.getNextActiveCellUseCase = getNextActiveCellUseCase;
exports.setActiveCellUseCase = setActiveCellUseCase;
exports.setCellsValueUseCase = setCellsValueUseCase;
exports.setExtraInputsUseCase = setExtraInputsUseCase;
exports.setStarDetailsUseCase = setStarDetailsUseCase;
exports.setVacDapUseCase = setVacDapUseCase;
//# sourceMappingURL=getNextActiveCell.useCase-3565e781.js.map
