import { SensoryLevels, MotorLevels, ValidSensoryValues, ValidMotorValues } from './core/domain/index.js';
...
const validateExamData = (examData) => {
    ...
    SensoryLevels.forEach((level) => {
        validateValue(`rightLightTouch${level}`, examData, errors, ValidSensoryValues);
        validateValue(`rightPinPrick${level}`, examData, errors, ValidSensoryValues);
        validateValue(`leftLightTouch${level}`, examData, errors, ValidSensoryValues);
        validateValue(`leftPinPrick${level}`, examData, errors, ValidSensoryValues);
        if (MotorLevels.includes(level)) {
...
const getRow = (level, examData) => {
    const isMotor = MotorLevels.includes(level);
...
export { formatCompleteIncomplete as a, formatLevelName as b, findCell as c, getCellRange as d, getCellPosition as e, formatASIAImpairmentScale as f, getEmptyExamData as g, motorCellRegex as h, getExamDataFromGridModel as i, sensoryValueRegex as j, getCellColumn as k, getCellRow as l, motorValueRegex as m, getCellComments as n, levelNameRegex as o, getEmptyTotals as p, bindExamDataToGridModel as q, bindExamDataToTotals as r, sensoryCellRegex as s, getExamDataWithAllNormalValues as t, cellLevelRegex as u, validateExamData as v, lightTouchCellRegex as w, pinPrickCellRegex as x, validCellNameRegex as y, cloneExamData as z };
//# sourceMappingURL=examData.helper-51d9ca17.js.map
